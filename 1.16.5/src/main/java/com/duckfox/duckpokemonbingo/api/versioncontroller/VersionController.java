package com.duckfox.duckpokemonbingo.api.versioncontroller;
...
public class VersionController implements IVersionController {
    ...
    public Object getSpeciesFromNameAnyCase(String name) {
        ...
        for (Species species : PixelmonSpecies.getAll()) {
            if (species.getName().equalsIgnoreCase(name) || species.getLocalizedName().equalsIgnoreCase(name)) {
                return species;
            }
        }
        return null;
    ...
    @Override
    public boolean isSpecies(Pokemon pokemon, Object species) {
        return pokemon.getSpecies().equals(species);
    }
}
